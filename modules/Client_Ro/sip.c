/**
 * $Id: sip.c 1049 2011-03-22 15:21:29Z aon $
 *  
...
str cscf_get_call_id(struct sip_msg *msg, struct hdr_field **hr) {
    ...
    if (parse_headers(msg, HDR_CALLID_F, 0) < 0) {
        LM_ERR("ERR:"M_NAME":cscf_get_call_id: error parsing headers\n");
        return call_id;
    }
    h = msg->callid;
    if (!h) {
        LM_ERR("ERR:"M_NAME":cscf_get_call_id: Header Call-ID not found\n");
        return call_id;
    }
    if (hr) *hr = h;
    call_id = h->body;
...
int cscf_get_from_uri(struct sip_msg* msg, str *local_uri) {
    ...
    if (!msg || parse_from_header(msg) < 0 || !msg->from || !msg->from->parsed) {
        LM_ERR("ERR:"M_NAME":cscf_get_from_uri: error parsing From header\n");
        if (local_uri) {
            local_uri->s = 0;
            local_uri->len = 0;
        }
...
int cscf_get_to_uri(struct sip_msg* msg, str *local_uri) {
    ...
    if (!msg || !msg->to || !msg->to->parsed || parse_headers(msg, HDR_TO_F, 0) == -1) {
        LM_ERR("ERR:"M_NAME":cscf_get_to_uri: error parsing TO header\n");
        if (local_uri) {
            local_uri->s = 0;
            local_uri->len = 0;
        }
...
int cscf_get_expires_hdr(struct sip_msg *msg, int is_shm) {
    ...
    if (parse_headers(msg, HDR_EXPIRES_F, 0) != 0) {
        LM_ERR("ERR:"M_NAME":cscf_get_expires_hdr: Error parsing until header EXPIRES: \n");
        return -1;
    }
    if (msg->expires) {
        ...
        if (msg->expires->parsed) {
            ...
            if (exp->valid) {
                expires = exp->val;
                LM_ERR("DBG:"M_NAME":cscf_get_expires_hdr: <%d> \n", expires);
                if (is_shm) {
    ...
    while (h) {
        if (h->name.len == s_asserted_identity.len &&
            ...
            if (parse_rr(h) < 0) {
                ...
                LM_CRIT("WARN:"M_NAME":cscf_get_asserted_identity: P-Asserted-Identity header must contain a Nameaddr!!! Fix the client!\n");
                id.name.s = h->body.s;
                id.name.len = 0;
                id.len = h->body.len;
                id.uri = h->body;
            ...
            free_rr(&r);
            h->parsed = r;
            //LM_CRIT(L_CRIT,"%.*s",id.uri.len,id.uri.s);
            return id.uri;
        }
        h = h->next;
    }
...
struct hdr_field* cscf_get_header(struct sip_msg * msg, str header_name) {
    ...
    /*if (parse_headers(msg, HDR_EOH_F, 0)<0){
            LM_ERR("ERR:"M_NAME":cscf_get_path: error parsing headers\n");
            return NULL;
    }*/
    h = msg->headers;
    while (h) {
...
loop:
    ...
    if (strncmp(p, "icid-value", 10) == 0) {
        p = strtok(NULL, " ;:\r\t\n\"=");
        if (p > (header_body.s + header_body.len)) {
            LM_ERR("ERR:"M_NAME":cscf_get_p_charging_vector: no value for icid\n");
            return 0;
        }
        temp.s = p;
        temp.len = 0;
    ...
    return 1;
out_of_memory:
    LM_ERR("ERR:"M_NAME":cscf_get_p_charging_vector:out of pkg memory\n");
    return 0;
}
