...
static inline int Ro_add_avp(AAAMessage *m, char *d, int len, int avp_code, int flags, int vendorid, int data_do, const char *func) {
    ...
    avp = cdp_avp->cdp->AAACreateAVP(avp_code, flags, vendorid, d, len, data_do);
    if (!avp) {
        LM_ERR("ERR:"M_NAME":%s: Failed creating avp\n", func);
        return 0;
    }
    if (cdp_avp->cdp->AAAAddAVPToMessage(m, avp, m->avpList.tail) != AAA_ERR_SUCCESS) {
        LM_ERR("ERR:"M_NAME":%s: Failed adding avp to message\n", func);
        cdp_avp->cdp->AAAFreeAVP(&avp);
        return 0;
    }
    return 1;
}
...
inline int Ro_add_multiple_service_credit_Control_stop(AAAMessage *msg, int used_unit) {
    ...
    if (used_unit >= 0) {
        ...
        used_group = cdp_avp->cdp->AAAGroupAVPS(used_list);
        cdp_avp->cdp->AAAFreeAVPList(&used_list);
    ...
    used_group = cdp_avp->cdp->AAAGroupAVPS(mscc_list);
    cdp_avp->cdp->AAAFreeAVPList(&mscc_list);
...
inline int Ro_add_multiple_service_credit_Control(AAAMessage *msg, unsigned int requested_unit, int used_unit) {
    ...
    group = cdp_avp->cdp->AAAGroupAVPS(mscc_list);
    cdp_avp->cdp->AAAFreeAVPList(&mscc_list);
...
void send_ccr_interim(struct ro_session* ro_session, str* from_uri, str *to_uri, int *new_credit, int *credit_valid_for, unsigned int used, unsigned int reserve, unsigned int *is_final_allocation) {
    ...
    auth = cdp_avp->cdp->AAAGetAuthSession(ro_session->ro_session_id);
    ...
    if (!auth) {
        ...
        auth = cdp_avp->cdp->AAAMakeSession(ro_session->auth_appid, ro_session->auth_session_type, ro_session->ro_session_id); //TODO: would like this session to last longer (see session timeout in cdp
        if (!auth)
            goto error;
    }
...
void remove_aaa_session(str *session_id) {
    ...
    if ((session = cdp_avp->cdp->AAAGetSession(*session_id))) {
